...
void ZoomToRectAction::Destroy()
{
    m_rectComponent.Reset();
}


bool ZoomToRectAction::AcceptTouchStream()
{
    return (m_numTouchStreams == 0);
}


ActionCommand ZoomToRectAction::HandleTouchEvent(ASI* pSI, TouchEvent* pEvent)
{
    ...
    else if (eventType == TouchEvent::Type_Release)
    {
        ...
        pSI->FitRectInView(m_zoomRect.Rect());
...
void ZoomToRectAction::HandleViewTransformed(ASI* pSI)
{
...
void ZoomToRectAction::UpdateRectComponent(ASI* pSI)
{
    DRef dref = pSI->NewData(BOI_STD_D(Size));
    *dref.GetWriteInstance<QSizeF>() = m_zoomRect.Rect().size();

    pSI->EmitTo(m_rectComponent, m_setSizeReceiver, dref);

    pSI->SetPosition(m_rectComponent, m_zoomRect.Rect().topLeft());
}
